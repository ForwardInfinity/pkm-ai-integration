-- Migration 006: Gate chunk-based search on embedding_status
-- Only search notes with completed embeddings to prevent stale/partial results
-- Fixes issue where semantic queries could surface notes with pending/processing/failed embeddings

-- ============================================================================
-- HYBRID SEARCH (gated on embedding_status = 'completed')
-- ============================================================================
...
as $$
  ...
  -- Only include notes with completed embeddings
  semantic_chunks as (
    select
      nc.note_id,
      nc.text_chunk,
...
-- ============================================================================
-- GET RELATED NOTES (gated on embedding_status = 'completed')
-- ============================================================================
...
as $$
  with target_chunks as (
    -- Only get chunks if target note has completed embeddings
    select embedding
    from note_chunks
...
-- ============================================================================
-- FIND POTENTIAL CONFLICTS (gated on embedding_status = 'completed')
-- ============================================================================
